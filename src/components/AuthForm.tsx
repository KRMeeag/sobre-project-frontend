...
export default function AuthForm() {
  ...
  useEffect(() => {
    if (selectedCodes.province) {
      getCities(selectedCodes.province)
        .then(setCitiesList)
        .catch(console.error);
    } else {
      setCitiesList([]);
  ...
  useEffect(() => {
    if (selectedCodes.city) {
      getBarangays(selectedCodes.city)
        .then(setBarangaysList)
        .catch(console.error);
    } else {
      setBarangaysList([]);
    }
  ...
  const validateStep1 = () => {
    ...
    if (!formData.fullName.trim()) {
      ...
      newErrors.fullName = "Name must contain only letters"; // <--- NEW VALIDATION
    }
      ...
      <div className="w-full max-w-90 flex flex-col">
        <div className="text-center mb-4">
          ...
          <h1 className="text-[33px] font-['Raleway'] font-extrabold text-[#223843] leading-[1.1] mb-2 px-2">
            {isSignUp
              ? "Create a Manager's Account"
              : "Data Driven Insights with Sobre"}
          </h1>
          ...
          {isSignUp && step === 1 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
              <InputField
                ...
                onChange={(e) => {
                  // Regex replaces anything that is NOT a letter (a-z, A-Z) or a space (\s) with an empty string
          ...
          {isSignUp && step === 2 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <InputField
                ...
                placeholder="Unit 101, Blue Bldg"
              ...
              <SelectField
                ...
                onChange={(e) => {
                  const code = e.target.value;
                  const name =
                    citiesList.find((c) => c.code === code)?.name || "";
              ...
              <SelectField
                ...
                onChange={(e) => {
                  const code = e.target.value;
                  const name =
                    provincesList.find((p) => p.code === code)?.name || "";
                  setSelectedCodes((prev) => ({
                    ...prev,
                    province: code,
                    city: "",
          ...
          <div className="pt-2 space-y-3">
            {!isSignUp ? (
              ...
              <div className="flex gap-2">
                ...
                <Button
                  className="w-2/3"
                  text={loading ? "..." : "Create Manager's Account"}
                  disabled={loading}
                />
              </div>
            )}
        ...
        <div className="mt-4 text-center pt-4 border-t border-gray-100">
          <p className="text-[11px] text-gray-500 mb-4 font-['Work_Sans']">
            {isSignUp
              ...
              : "Don't have an account? Click the button below!"}
          </p>
          <button
            type="button"
        ...
        {isSignUp && (
          <div className="flex justify-center gap-2 mt-4">
            <div
              className={`w-2 h-2 rounded-full transition-all ${step === 1 ? "bg-[#004385] w-4" : "bg-gray-300"}`}
            />
            <div
              className={`w-2 h-2 rounded-full transition-all ${step === 2 ? "bg-[#004385] w-4" : "bg-gray-300"}`}
            />
          </div>
        )}
      </div>
...
const InputField = ({ label, error, ...props }: InputFieldProps) => (
  <div className="flex flex-col">
    ...
    />
    {error && (
      <span className="text-red-500 text-[10px] ml-1 mt-0.5">{error}</span>
    )}
  </div>
);
