...
export default function AddItemModal({
  ...
  const isFormValid =
    ...
    formData.primary_supplier.trim() !== "" && // CORRECTED
    formData.cost !== "" &&
    formData.price !== "";
  ...
  const handleSubmit = async () => {
    ...
    if (costNum > priceNum) {
      if (
        !window.confirm(
          `Heads up! The Cost (₱${costNum}) is greater than the Selling Price (₱${priceNum}).\n\nAre you sure you want to add this item?`,
        )
      )
        return;
    }
    ...
    try {
      ...
      await axios.post(`${API_URL}/inventory?users_id=${userId}`, {
        ...
        cost: costNum,
        price: priceNum,
        primary_supplier: formData.primary_supplier.trim(), // CORRECTED KEY
    ...
    <div className="fixed inset-0 bg-[#35435a]/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl my-auto relative animate-in fade-in zoom-in-95 duration-200 flex flex-col">
        ...
        <div className="px-8 pt-6 pb-4 flex justify-between items-center border-b border-gray-100 shrink-0 bg-white rounded-t-2xl">
          <h2 className="text-xl font-bold text-[#004385] font-['Raleway']">
            Add New Item
          </h2>
          <button
            onClick={handleClose}
        ...
        <div className="px-8 py-6 font-['Work_Sans'] bg-white">
          {error && (
            <div className="mb-6 p-3 bg-red-50 border border-red-200 text-[#b13e3e] text-sm font-medium rounded-lg">
              {error}
            </div>
          )}
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <ComboboxInput
                ...
                name="primary_supplier" // Corrected
                value={formData.primary_supplier} // Corrected
                options={existingSuppliers}
                onChange={handleTextChange}
                onSelect={(val) => {
                  setIsDirty(true);
                  setFormData((prev) => ({ ...prev, primary_supplier: val })); // Corrected
                }}
                required
              />
        ...
        <div className="px-8 py-5 border-t border-gray-100 flex justify-end shrink-0 bg-gray-50 rounded-b-2xl">
          <button
            onClick={handleClose}
            disabled={loading}
            className="text-gray-500 hover:bg-gray-200 font-medium py-2.5 px-6 rounded-lg transition-colors text-sm mr-3"
          >
            Cancel
          </button>
          <button
